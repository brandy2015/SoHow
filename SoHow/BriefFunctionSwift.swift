...
func 颜色(red:CGFloat,green:CGFloat,blue:CGFloat,alpha:CGFloat = 1.0) -> UIColor {
    return UIColor(red:red/255.0, green: green/255.0, blue: blue/255.0, alpha: alpha)
}



func 时间格式的调整(date:Date,format:String = "yyyy年MM月dd日 HH:mm:ss") -> String  {
    // 创建一个日期格式器
...
func 时间格式的调整2只剩年月日时间创建录音(date:Date) -> String  {
    // 创建一个日期格式器
    let dformatter = DateFormatter()
    // 为日期格式器设置格式字符串
...
func 返回设备信息() -> (String,String,String,String,String,String,String,String,[String]){
...
//MARK: - 压缩图片大小
func y压缩imageCompress(originalImage: UIImage) -> UIImage{
...
func share分享功能(文件地址:String,测试:Bool = false) -> UIActivityViewController{
    //    if let
...
func 删除http链接无关字符只留主题文字(str:String) -> String  {
    var str = str
    if str == ""{return "" }else if str.contains("http://"){
        str = 处理http(httpstring: str, 标志: "http://")
    }else if str.contains("https://"){
...
func 处理http(httpstring: String,标志:String) -> String {
...
func 有没有Htttps(httpstring:String) -> URL {
...
func 打开网页(网路WebView:UIWebView,地址:String)  {
...
extension String{
    ...
    func transformToPinYin() -> String{//(String,String){
